    ...
    class GeneratorTask:
        ...
        def simulate_Model(self, model, config):
            ...
            if self.generator.simulation_method == 'DEFact':
                ...
                process = init_DEFact(output_queue, config_space, print_events=True)
...
def DEFact_wrapper(
    ...
    initial_process: Process = init_DEFact(output_queue, config_space, print_events=print_events)

    all_features = []
    final_process = initial_process
    try:
        all_features, final_process = main_gedi_event_loop(input_params, n_windows, output_queue, window_size, initial_process, print_events)
        ...
        if final_process.is_alive():
            terminate_DEFact(final_process)
...
def abbreviate_key(key):
    # Split the key by underscore and take first letter of each part
    parts = key.split('_')
    abbreviated = '_'.join([part[0] for part in parts])
    return abbreviated


def print_results(all_features):
    if not all_features:
    ...
    abbreviated_headers = [abbreviate_key(key) for key in original_headers]
...
def main_gedi_event_loop(
    ...
    for window_num in range(1, n_windows + 1):
        ...
        terminate_DEFact(current_process)
        ...
        genED: GenerateEventLogs = GenerateEventLogs(params_for_gen)
        ...
        if window_num < n_windows:
            current_process = init_DEFact(output_queue, config.config_space, print_events=print_events)
